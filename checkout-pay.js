...
import htmlString        from './checkout-pay.html';
...
class CheckoutPay extends AppElement {
  static get is() { return 'checkout-pay'; }
  ...
  async __getAuthToken(uid) {
    if (uid) {
      ...
      // sometimes braintree doesnt like to send auth patch
      if (!authToken) {
        return braintreeConfig.authToken;
      }
      return authToken;
    }
    this._braintreeCustomerId   = null;
  ...
  async __payButtonClicked() {
    try {
      ...
      this.fire('save-error', {data: this._orderForPay,  func: 'checkout-pay __payButtonClicked'});
      const {code} = error;
  ...
  async open() {
    const {email, inventoryAdjustments, total, user} = this.order;
    ...
    if (!inventoryAdjustments) { 
      this.fire('save-error', {data: this.order,  func: 'checkout-pay open inventoryAdjustments'});
    }
    try {
      const amount = Number(total).toFixed(2);
      const uid    = user ? user.uid : null;
      this.$.payCardTitle.innerText = `Total $${amount}`;
      await import('./checkout-pay.css');
      this.__showSpinner();
      await schedule();
      ...
      this.fire('save-error', {data: this.order,  func: 'checkout-pay open'});
      this.__exit();
      console.error(error);
    }
  }
